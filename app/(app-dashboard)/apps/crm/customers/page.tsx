...
export default function CustomersPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/customer/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
      ...
      cell: ({ row }) => {
        return (
          <div className='flex flex-row gap-3'>
            <Button size={'sm'} asChild><Link href={'/apps/crm/customers/' + row.original.id}><EyeIcon /></Link></Button>
            <AlertDialog>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View all customer and their orders.</CardDescription>
        <div className='flex justify-end'>
          <Button onClick={() => router.push('/apps/crm/customers/new')}>New Customers</Button>
        </div>
      </CardHeader>
      <CardContent>
